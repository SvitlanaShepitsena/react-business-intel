...
function build(babel) {
  ...
  return new Transformer("typecheck", {
    ...
    Function: function Function(node, parent, scope) {
      ...
      if (node.typeParameters != null && node.typeParameters.type === 'TypeParameterDeclaration') {
        genericTypes.push.apply(genericTypes, _toConsumableArray(node.typeParameters.params.map(function (param) {
          return param.name;
        })));
      }
      var argumentGuards = createArgumentGuards(node, genericTypes);
   ...
   */
  function extractAnnotationTypes(_x10) {
    var _again = true;
    ...
    _function: while (_again) {
      ...
      switch (annotation.type) {
        case "TypeAnnotation":
          _x10 = annotation.typeAnnotation;
          _again = true;
          continue _function;
  ...
  function createArgumentGuards(node) {
    ...
    return node.params.reduce(function (guards, param) {
      if (param.type === "AssignmentPattern" && param.left.typeAnnotation) {
        ...
        var types = extractAnnotationTypes(param.typeAnnotation);
        if (!types.includes("array") || types.some(function (t) {
          return t !== "array";
  ...
  function createArgumentGuard(param, types) {
    ...
    var test = createIfTest(param, types, genericTypes);
    if (!test) {
      return null;
    }
    return t.ifStatement(test, t.throwStatement(t.newExpression(t.identifier("TypeError"), [t.binaryExpression("+", t.literal('Value of ' + (param.optional ? 'optional argument' : 'argument') + ' \'' + param.name + '\' violates contract, expected ' + createTypeNameList(types) + ' got '), createReadableTypeName(param))])));
  }
  ...
  function createReturnTypeGuard(ref, node, scope, state) {
    var test = createIfTest(ref, state.returnTypes, state.genericTypes);
    if (!test) {
      return null;
    }
    return t.ifStatement(test, t.throwStatement(t.newExpression(t.identifier("TypeError"), [t.binaryExpression("+", t.literal('Function ' + (state.subject.id ? '\'' + state.subject.id.name + '\' ' : '') + 'return value violates contract, expected ' + createTypeNameList(state.returnTypes) + ' got '), createReadableTypeName(ref))])));
  }
